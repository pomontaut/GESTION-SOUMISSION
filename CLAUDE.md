...
## Agent dédié : comparatif-tco

Second sous-agent spécialisé, `.claude/agents/comparatif-tco.md`, dédié à la future génération de comparatifs d'offres ("TCO") de qualité professionnelle à partir du suivi des offres (`FollowUpEntry`, onglet Suivi). **Toujours l'invoquer en priorité** pour : étudier de nouveaux exemples réels de comparatifs fournis par l'utilisateur, concevoir ou faire évoluer la structure/les critères d'un comparatif généré, ou expliquer ce qui distingue un bon comparatif d'un tableau générique. Cette fonctionnalité n'est pas encore implémentée (au 16.09.2026) - le rôle de cet agent est de construire méthodiquement la mémoire de domaine (structure, critères, formules observées sur des exemples réels) avant que le code de génération soit écrit.
